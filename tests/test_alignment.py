...
def test_denoiser_alignment(align_sim, align_templates):
    ...
    transformers = [
        ...
        dartsort.transform.Voltage(
            channel_index=ci, waveform_cfg=waveform_cfg, sampling_frequency=fs
        ),
    ]
    ...
    with tempfile.TemporaryDirectory() as tdir:
        ...
        sts = [st0, st1]
        wf0 = st0.load_dataset("collisioncleaned_waveforms")
        wf1 = st1.load_dataset("collisioncleaned_waveforms")
    ...
    sts = [st.ephemeral_replace(labels=(st.voltages < 0).astype(int)) for st in sts]
    assert len(sts) == 2
